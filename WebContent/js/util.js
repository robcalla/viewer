...
function getServicePathName(refScopeId, urbanserviceId){
	...
	var input = new Object();
	input['action'] = 'getCategoryName';
	input['context'] = refScopeId; 
	input['category'] = urbanserviceId; 
	...
	$.ajax({
		...
		data : input,
		success : function(data) {
			$('#urbanservice_breadcrumb').text(data);
		},
		error : function(xhr, status, error) {
			console.log(status);
		}
