...
public class AjaxHandler extends HttpServlet {
	...
	public enum Ajaxaction {
		createContext("createContext"), createCategory("createCategory"), getContexts("getContexts"),
		getCategories("getCategories"), getContextName("getContextName"), getCategoryName("getCategoryName"),

		getCockpitNames("getCockpitNames"), setCockpitName("setCockpitName"),

		deleteCategory("deleteCategory"), deleteCategoriesByContextName("deleteCategoriesByContextName"),
		deleteContext("deleteContext"), appendDashboard("appendDashboard"), removeDashboard("removeDashboard");
		...
		try {
			...
			case deleteCategoriesByContextName: {
				...
				if (!perm.getCanDelete())
					...
					serviceResponse = ServiceConfigManager.deleteCategoriesByContextName(context);
			...
			case deleteContext: {
				...
				if (!perm.getCanDelete())
					...
					serviceResponse = ServiceConfigManager.deleteContext(context);
			...
			case createCategory: {
				...
				boolean serviceResponse = ServiceConfigManager.createCategory(context, category, dashboardName, type,
						enabler);
			...
			case getContexts: {
				try {
					...
					if (!userIsAdmin && !isTenantActive) {
						Map<ServiceEntityBean, ResourcePermission> permittedServices = ServiceConfigManager
								.getPermittedServices(services, userInfo);
						result = ServiceConfigManager.addServiceInfo(permittedServices);
					} else if (!userIsAdmin && isTenantActive) {
						Map<ScopeEntityBean, ResourcePermission> permittedScopes = ServiceConfigManager
								.getPermittedScopes(scopes, userInfo);
			...
			case getCockpitNames: {
				...
				if (isAdmin) {
					...
					perm = pManager.getContextPermission(userInfo, scopeResource);
					if (perm.getCanRead()) {
						// TODO In this version in order to obtain category permission you have to
						// invoke the corrispective methods using as resource the scope
			...
			case appendDashboard: {
				...
				serviceResponse = ServiceConfigManager.appendCockpitFromPersistence(entityId, cockpitName, cockpitUrl,
						type);
				jResp.put("orion_update", serviceResponse);
				response.getWriter().write(jResp.toString());
				break;
